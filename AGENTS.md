...
## Qué es
Agente de overlay de cobertura con **opciones** para el
...
- `src/options_agent/`
  ...
  - `alpaca/` — `client.py` (paper), `options.py` (contratos/cadena), `orders.py` (tickets)
  - `strategy/` — `base.py` (vocabulario), `overlay.py` (protective put; intercambiable)
  - `risk/` — `limits.py` (por orden), `account.py` (circuit breaker de cuenta)
  - `agent/` — `loop.py` (ciclo), `tools.py` (tools del LLM), `llm.py` (advisor opcional)
...
- El LLM propone; `risk/` decide. No crear caminos que salten el riesgo.
- Short desnudo de opciones: prohibido.
